...
def can_all_clients_complete(allClientStatus):
    """
    tell if all clients can complete or not
    """

    canComplete = True

    for b in allClientStatus:
        if not b:
            canComplete = False
...
def is_request_safe(request, numClient, E_TotalResources, R_MaximumRequest, C_CurrentAllocations, A_AvailableResources):
    """
    determine if the request can be satisfied by loans being paid back to the bank.
    can we complete all client requests with available resources
    """
    ...
    # make a list of booleans, assume all clients can not complete
    canPayOffClientStatus = [False] * numClient

    # the order the clients can complete
    clientCompleteOrder = []

    # check all the clients until you do not need to check again, O(N^2) algorithm
    for outer in range(0, numClient):
        ...
        for client in range(0, numClient):
            ...
            if canPayOff and (not canPayOffClientStatus[client]):
                potentialAvailableResources = list(map(int.__add__, potentialAvailableResources, C_CurrentAllocations[client]))
                canPayOffClientStatus[client] = True
                clientCompleteOrder += [client]
    ...
    if can_all_clients_complete(canPayOffClientStatus):
        print("Clients can complete in the order:" + str(clientCompleteOrder))
...
while not userQuit:
    ...
    if (not userQuit) and validClient:
        ...
        safe = is_request_safe(request, numClient, E_TotalResources, R_MaximumRequest, C_CurrentAllocations, A_AvailableResources)
